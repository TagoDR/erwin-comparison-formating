...
function un(e) {
	let t = new DOMParser().parseFromString(e, "text/html").querySelectorAll("tbody tr"), n = null, r = [];
	return t.forEach((e) => {
		let t = e.querySelectorAll("td");
		if (t.length < 4) return;
		let i = t[0], a = t[1].textContent?.trim() || "", o = t[3].textContent?.trim() || "", s = i.textContent || "", c = s.trim(), l = (s.match(/^[\s\u00a0]*/)?.[0] || "").length, u = Math.floor(l / 3), d = ln.includes(c), f = cn.find((e) => e.object === c && e.indentation.includes(l)), p = {
			level: u,
			spaces: l,
			type: c,
			left: a,
		...
		if (f) {
			...
			if (!n && c === "Model") n = e, r.push({
				obj: n,
				level: u
			});
			else {
				for (; r.length > 0 && r[r.length - 1].level >= u - 1;) r.pop();
				if (r.length > 0) {
					let t = r[r.length - 1], n = t.lastGrouping || "Other";
					t.obj.children || (t.obj.children = {}), t.obj.children[n] || (t.obj.children[n] = []), t.obj.children[n].push(e), r.push({
						obj: e,
						level: u
					});
				}
			...
			for (; r.length > 0 && r[r.length - 1].level >= u;) r.pop();
			r.length > 0 && (r[r.length - 1].lastGrouping = c);
		} else {
			for (; r.length > 0 && r[r.length - 1].level >= u;) r.pop();
			if (r.length > 0) {
				let e = r[r.length - 1].obj;
