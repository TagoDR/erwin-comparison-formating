	...
	*/
	function parseErwinHtml(html) {
		...
		doc.querySelectorAll("tbody tr").forEach((tr, index) => {
			...
			let change = "";
			if (leftClean && rightClean) change = "A";
			else if (leftClean) change = "I";
			else if (rightClean) change = "E";
			const isUDP = (type.startsWith("Entity.Physical.") || type.startsWith("Entity.Logical.")) && spaces === 15 || (type.startsWith("Attribute.Physical.") || type.startsWith("Attribute.Logical.")) && spaces === 18;
			rows.push({
				id: `row-${index}`,
				type,
				level,
	...
	var rawData$, isLoading$, fileName$, isUserscript$, filterChange$, filterName$, onlyEntities$, onlyEntitiesAndAttributes$, hideCalculated$, showProperties$, toggledPropertiesIds$, hiddenSubObjectsIds$, checkedIds$, isFlipped$, enrichedData$, filteredData$, togglePropertiesGlobal, togglePropertiesIndividual, toggleSubObjects, toggleCheck, initializeVisibility, resetFilters, toggleFlip, statsSummary$;
	var init_data_store = __esmMin((() => {
		...
		enrichedData$ = /* @__PURE__ */ computed(rawData$, (data) => {
			...
			const processed = data.map((row, index) => {
				...
				const headerMatch = HEADERS_CONFIG.find((h) => h.object === cleanedType && h.indentation.includes(row.spaces));
				const enriched = {
					...row,
					id: row.id || `row-${index}`,
					...
					isCalculated: row.left === row.right || row.left.endsWith("[Calculated]") && row.right.endsWith("[Calculated]")
				};
				if (!enriched.view) {
				...
				while (headerStack.length > 0 && headerStack[headerStack.length - 1].level >= enriched.level) headerStack.pop();
				let parentId = "";
					...
					if (childrenIds.length > 0) {
						...
						if (row.type === "Entity/Table" || row.type === "Entity" || row.type === "Table") for (const cid of childrenIds) {
							...
							if (child?.type === "Column Order List" || child?.type === "Attribute Order List") {
								const count = Math.max(child.left ? child.left.split(",").length : 0, child.right ? child.right.split(",").length : 0);
